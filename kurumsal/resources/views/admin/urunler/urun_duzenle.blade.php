                                     ...
                                     <div class="row mb-3">
                                        <div class="col-sm-12 form-group">
                                            <label for="example-text-input" class="col-form-label">Metin</label>
                                                    <textarea id="elm1" name="metin" type="text">{{ $urunler->metin }}</textarea>                                                
                                        </div>
                                    </div>
                                            ...
                                            <div class="col-sm-12">
                                                <select class="form-select" aria-label="Default select example" name ="kategori_id">
                                                  ...
                                                  @foreach($kategoriler as $kategori)
                                                    <option value="{{ $kategori->id }}" {{ $urunler->kategori_id == $kategori->id ? 'selected' : '' }}>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-12 form-group">
                                    <select class="form-select" aria-label="Default select example" name="altkategori_id">
                                        <option value="" selected>Alt Kategori Seç</option>

                                        @foreach($altkategoriler as $altkategori)
                                            <option value="{{ $altkategori->id }}" {{ $urunler->altkategori_id == $altkategori->id ? 'selected' : '' }}>
            ...
            if (kategori_id) {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('select[name="altkategori_id"]').append('<option value="" selected>Alt Kategori Seç</option>');
                        $.each(data, function(key, value) {
                            $('select[name="altkategori_id"]').append('<option value="'+ value.id +'">' + value.altkategori_adi + '</option>');
                        });
                        $('select[name="altkategori_id"]').trigger('change');
                    },
                });
            } else {
                alert('danger');
